...
export default function Dashboard({
    ...
    const handleSearchChange = (e) => {
        ...
        const filteredBrandIds = new Set(
            filteredProducts.map((p) => p.brand_id)
        );
        const filteredCategoryIds = new Set(
            filteredProducts.map((p) => p.category_id)
        );

        const filteredBrands = brands.data.filter(
            (brand) =>
                (brand.name && brand.name.toLowerCase().includes(trimmed)) ||
                (brand.description &&
                    brand.description.toLowerCase().includes(trimmed)) ||
                filteredBrandIds.has(brand.id)
        );
            ...
            <div>
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {(search ? searchResults.products : products.data).map(
                            (product) => (
                                <div
                                    key={product.id}
                    ...
                    <section className="mt-12">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                            {(search ? searchResults.brands : brands.data).map(
                                (brand) => (
                                    <div
                                        key={brand.id}
                    ...
                    <section className="mt-12">
                        <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                            ...
                            />
                            Categorías{" "}
                            {search && `relacionadas con "${search}"`}
                        </h2>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                            {(search
                                ...
                                >
                                    <div className="relative group">
                                        <div className="w-full aspect-[3/2] overflow-hidden rounded-md bg-gray-100">
                                            <img
                                                src={
                                                    category.image?.url
                                                        ? `/storage/${category.image.url}`
                                                        : "https://via.placeholder.com/300x180.png?text=Categoría"
                                                }
                                                alt={category.name}
                                                className="w-full h-full object-cover"
                                            />
